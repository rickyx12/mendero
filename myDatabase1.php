...
if( $type == "IPD" ) {
$result = mysql_query("SELECT count(registrationNo) as regNo FROM registrationDetails WHERE (dateRegistered between '$date' and '$date1') and type in ('IPD','ER','ICU','OR/DR')  ");
}else {
...
$con = mysql_connect($this->myHost,$this->username,'july022015');
if (!$con)
  {
  die('Could not connect: ' . mysql_error());
  }
...
if( $type == "All" ) {
$result = mysql_query("select pc.chargesCode,pc.description,sum(pc.quantity) as qtyDispensed from patientCharges pc,registrationDetails rd WHERE pc.registrationNo = rd.registrationNo and (pc.dateCharge between '$date1' and '$date2') and pc.title = '$title' and pc.departmentStatus like 'dispensedBy%%%%' and pc.status not like 'DELETED_%%%%%%' and departmentStatus like 'dispensedBy%%%%%' group by pc.chargesCode order by qtyDispensed desc ");
}else {

if( $type == "IPD" ) {
$result = mysql_query("select pc.chargesCode,pc.description,sum(pc.quantity) as qtyDispensed from patientCharges pc,registrationDetails rd WHERE pc.registrationNo = rd.registrationNo and rd.type IN ('IPD','ER','ICU','OR/DR') and (pc.dateCharge between '$date1' and '$date2') and pc.title = '$title' and pc.departmentStatus like 'dispensedBy%%%%' and pc.status not like 'DELETED_%%%%%%' and departmentStatus like 'dispensedBy%%%%%%%%' group by pc.description order by qtyDispensed desc ");
}else {
$result = mysql_query("select pc.chargesCode,pc.description,sum(pc.quantity) as qtyDispensed from patientCharges pc,registrationDetails rd WHERE pc.registrationNo = rd.registrationNo and rd.type ='$type' and (pc.dateCharge between '$date1' and '$date2') and pc.title = '$title' and pc.departmentStatus like 'dispensedBy%%%%' and pc.status not like 'DELETED_%%%%%%' and departmentStatus like 'dispensedBy%%%%%%%%' group by pc.description order by qtyDispensed desc ");
}
...
$this->coconutTableRowStart();
$this->coconutTableData("".$x++);
$this->coconutTableData($row['chargesCode']);
$this->coconutTableData($row['description']);
...
if( $type == "All" ) {
$result = mysql_query("select sum(pc.quantity) as qtyDispensed from patientCharges pc,registrationDetails rd WHERE pc.registrationNo = rd.registrationNo and pc.dateCharge = '$date' and pc.description = '$description' and pc.chargesCode = '$chargesCode' and pc.departmentStatus like 'dispensedBy%%%%' and pc.status not like 'DELETED%%%%%%%' ");
}else {
$result = mysql_query("select sum(pc.quantity) as qtyDispensed from patientCharges pc,registrationDetails rd WHERE pc.registrationNo = rd.registrationNo and rd.type ='$type' and pc.dateCharge  = '$date' and pc.chargesCode = '$chargesCode' and pc.description = '$description' and pc.departmentStatus like 'dispensedBy%%%%' and pc.status not like 'DELETED%%%%%%' ");
}
...
$result = mysql_query("select pc.itemNo,pc.description,pc.total,pc.datePaid,pc.cashPaid,pc.orNO,title,quantity from patientCharges pc,registrationDetails rd WHERE pc.registrationNo = rd.registrationNo and pc.registrationNo = '$registrationNo' and pc.status = 'PAID' order by pc.orNO asc ");
...
$this->coconutTableData("<input type='checkbox' name='itemNo[]' value='".$row['itemNo']."_".$row['total']."_".$row['orNO']."' >");
...
$this->coconutTableData("<a href='http://".$this->getMyUrl()."/COCONUT/patientProfile/voidPayment/returnItem.php?itemNo=$row[itemNo]&registrationNo=$registrationNo&username=$user&orNO=$row[orNO]' style='color:red; size=2'>[Void]</a>");
}else {
$this->coconutTableData("");
}
...
public function AddVoidPayment($patientName,$item,$amount,$time,$date,$user,$orNo) {
...
$sql="INSERT INTO voidPayment (patientName,item,amount,timeVoid,dateVoid,voidBy,orNo)
VALUES
('".mysql_real_escape_string($patientName)."','".mysql_real_escape_string($item)."','".mysql_real_escape_string($amount)."','".mysql_real_escape_string($time)."','".mysql_real_escape_string($date)."','".mysql_real_escape_string($user)."','$orNo')";
...
while($row = mysql_fetch_array($result))
  {
$this->coconutTableRowStart();
$this->coconutTableData("&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/Laboratory/resultList/resultFormTemplateTest.php?username=$username&registrationNo=$registrationNo&chargesCode=$chargesCode&itemNo=$itemNo&templateNo=$row[templateNo]'>".$row['title']."</a>&nbsp;");
$this->coconutTableRowStop();
}
$this->coconutTableStop();
}
...
if( $batchNo != "" ) {
$result = mysql_query("SELECT itemNo,description,quantity,sellingPrice,cashUnpaid,dateCharge,discount,chargeBy FROM patientCharges WHERE registrationNo = '$registrationNo' and batchNo = '$batchNo' and status = 'UNPAID' and (phic = 0 or company = 0) order by description asc ");
}else {
$result = mysql_query("SELECT itemNo,description,quantity,sellingPrice,cashUnpaid,dateCharge,discount,chargeBy FROM patientCharges WHERE registrationNo = '$registrationNo' and status = 'UNPAID' and (phic = 0 or company = 0) order by description asc ");
}
...
$this->coconutTableHeader("Total");
$this->coconutTableHeader("Charge By");
$this->coconutTableRowStop();
while($row = mysql_fetch_array($result))
  {
