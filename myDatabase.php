...
if($room[1] == "OR/DR" || $room[1] == "OR" || $room[1] == "SUITE" || $room[1] == "OB-Package") {
...
$result = mysql_query("SELECT Category,chargesCode,upper(Description) as Description,(".$room[1].") as sellingPrice,Service FROM availableCharges where Description like '".$charges."%%%%%%%' ");
}
...
$sellingPrice = $row['sellingPrice'] + ( $row['sellingPrice'] * 0 ); //added apr 5, 2016
}else {
...
echo "|&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/availableCharges/addCharges.php?status=UNPAID&registrationNo=$registrationNo&chargesCode=$row[chargesCode]&description=$row[Description]&sellingPrice=$sellingPrice&discount=0&timeCharge=$serverTime&chargeBy=$username&service=Examination&title=$row[Category]&paidVia=Cash&cashPaid=0.00&batchNo=$batchNo&username=$username&quantity=1&inventoryFrom=none&room=".$room[0]."_".$room[1]."&paycash=yes'><font color=red>Pay Cash</font></a>&nbsp;";
...
public function deleteRoom($registrationNo,$username) {
...
mysql_query("UPDATE patientCharges SET status='DELETED_$username".$datetime."' WHERE title ='Room And Board' and registrationNo='$registrationNo' ");
...
if($show == "All") {
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and (status = 'UNPAID' or status = 'Return') and pendingDelete = '' order by dateCharge,timeCharge asc ");
}else {
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and (status = 'UNPAID' or status = 'Return') and description like '$desc%%%%%%' and pendingDelete = '' order by description asc ");
}
...
$title = $row['title'];
if($title=="LABORATORY" || $title=="LABORATORY"){
...
$result = mysql_query("SELECT * FROM patientCharges where registrationNo = '$registrationNo' and title='$title' and (status = 'UNPAID' or status = 'Discharged' or status = 'Return') and pendingDelete = '' order by dateCharge,timeCharge asc ");
...
if(( $this->selectNow("registeredUser","module","username",$username) == "ADMIN" )||( $this->selectNow("registeredUser","module","username",$username) == "BILLING" )||( $this->selectNow("registeredUser","module","username",$username) == "CASHIER" )||( $this->selectNow("registeredUser","module","username",$username) == "PHILHEALTH" )) {
//echo "<Td>&nbsp;<img src='http://".$this->getMyUrl()."/COCONUT/myImages/locked1.jpeg' />&nbsp;</tD>";
...
.style2 {font-family: Arial;font-size: 12px;color: #0066FF;}
.style3 {font-family: Arial;font-size: 12px;color: #FF0000;}
</style>
";
...
//1
//echo  "<th bgcolor='#3b5998'>&nbsp;<font color=white>Account Status</font>&nbsp;</th>";
echo "</tr>";
while($row = mysql_fetch_array($result))
  {
echo "<tr>";
echo "<td>&nbsp;<a href='/Registration/checkingStop.php?patientNo=$row[patientNo]&date=".date("y-m-d")."&from=$from'>".strtoupper($row['completeName'])."</a>&nbsp;</td>";
...
//1
/*$totac=0;
$regsql=mysql_query("SELECT registrationNo, discount FROM registrationDetails WHERE patientNo='".$row['patientNo']."' ORDER BY dateRegistered");
while($regfetch=mysql_fetch_array($regsql)){
$regNo=$regfetch['registrationNo'];
$discount=$regfetch['discount'];

$asql=mysql_query("SELECT SUM(cashUnpaid) AS unpaid, SUM(phic) AS phic, SUM(company) AS comp, SUM(cashPaid) AS paid FROM patientCharges WHERE registrationNo='$regNo' AND status NOT LIKE 'DELETED_%%%%'");
while($afetch=mysql_fetch_array($asql)){$unpaid=$afetch['unpaid'];$phic=$afetch['phic'];$comp=$afetch['comp'];$paid=$afetch['paid'];}

$bsql=mysql_query("SELECT SUM(cashUnpaid) AS unpaidud, SUM(phic) AS phicud, SUM(company) AS compud, SUM(cashPaid) AS paidud FROM patientCharges WHERE registrationNo='$regNo' AND status NOT LIKE 'DELETED_%%%%' AND title='MEDICINE' AND departmentStatus=''");
while($bfetch=mysql_fetch_array($bsql)){$unpaidud=$bfetch['unpaidud'];$phicud=$bfetch['phicud'];$compud=$bfetch['compud'];$paidud=$bfetch['paidud'];}

if($unpaidud==''){$realunpaidud=0;}else{$realunpaidud=$unpaidud;}

$csql=mysql_query("SELECT SUM(amountPaid) patAP FROM patientPayment WHERE registrationNo='$regNo' AND paymentFor NOT LIKE 'REFUND'");
while($cfetch=mysql_fetch_array($csql)){$patAP=$cfetch['patAP'];}*/



//$dsql=mysql_query("SELECT SUM(amountPaid) comAP, SUM(tax) AS comTAX, SUM(discount) AS comDISC FROM companyPayment WHERE registrationNo='$regNo'");
//while($dfetch=mysql_fetch_array($dsql)){$comAP=$dfetch['comAP'];$comTAX=$dfetch['comTAX'];$comDISC=$dfetch['comDISC'];}

//$esql=mysql_query("SELECT SUM(amountPaid) phicAP, SUM(tax) AS phicTAX FROM phicPayment WHERE registrationNo='$regNo'");
//while($efetch=mysql_fetch_array($esql)){$phicAP=$efetch['phicAP'];$phicTAX=$efetch['phicTAX'];}


//1
/*$refund = $this->doubleSelectNow("patientPayment","amountPaid","registrationNo",$regNo,"paymentFor","REFUND");
if($refund==''){$realrefund=0;}else{$realrefund=$refund;}
$runpaid=round($unpaid,2);
$rrealunpaidud=round($realunpaidud,2);
$rpatAP=round($patAP,2);
$rdiscount=round($discount,2);

$totac+=(($runpaid-$rrealunpaidud-$rdiscount)-($rpatAP-$realrefund));*/



//$zsql=mysql_query("SELECT module FROM registeredUser WHERE username='$username' AND (module='BILLING' OR module='CASHIER' OR module='PHARMACY' OR module='PHILHEALTH' OR module='SUPERVISOR')");
//$zcount=mysql_num_rows($zsql);

//if($zcount==0){

//1
/*if((($unpaid-$unpaidud)-$patAP)==0){echo "<span class='style2'>&nbsp;Fully Paid&nbsp;</span>";}
else if((($unpaid-$unpaidud)-$patAP)<0){echo "<span class='style2'>&nbsp;Inquire at Accounting&nbsp;</span>";}
else if((($unpaid-$unpaidud)-$patAP)>0.001){echo "<span class='style3'>&nbsp;Balance&nbsp;</span>";}*/
...
/*else if($zcount!=0){
if($unpaid!=0){echo "<span class='style1'>&nbsp;Charged: </span><span class='style2'>".number_format(($unpaid-$unpaidud),2)."</span>";}
if($patAP!=0){echo "<span class='style1'>&nbsp;Paid: </span><span class='style3'>".number_format($patAP,2)."</span>";}
if(is_numeric($row['discount'])){echo "<span class='style1'>Disc.: </span><span class='style3'>".number_format($row['discount'],2)."</span>";}
if((!is_numeric($row['discount']))&&($patAP==0)&&($unpaid==0)){}else{echo "<br>";}

if($comp!=0){echo "<span class='style1'>&nbsp;Company: </span><span class='style2'>".number_format(($comp-$compud),2)."</span>";}
if(($comAP+$comTAX)!=0){echo "<span class='style1'>&nbsp;Paid: </span><span class='style3'>".number_format(($comAP+$comTAX),2)."</span>";}
if(is_numeric($row['companyDiscount'])){echo "<span class='style1'>&nbsp;Disc.:&nbsp;</span><span class='style3'>".number_format($row['companyDiscount'],2)."</span>";}
if((!is_numeric($row['companyDiscount']))&&(($comAP+$comTAX)==0)&&($comp==0)){}else{echo "<br>";}

if($phic!=0){echo "<span class='style1'>&nbsp;PHIC: </span><span class='style2'>".number_format(($phic-$phicud),2)."</span>";}
if(($phicAP)!=0){echo"<span class='style1'>&nbsp;Paid: </span><span class='style3'>".number_format(($phicAP+$phicTAX),2)."</span>";}
if(($phic==0)&&(($phicAP)==0)){}else{echo "<br>";}

if($paid!=0){echo "<span class='style1'>&nbsp;Cash: </span><span class='style3'>".number_format(($paid-$paidud),2)."</span>";}
...
//1
/*if($totac==0){echo "<span class='style2'>&nbsp;Fully Paid&nbsp;</span>";}
else if($totac<0.001){echo "<span class='style2'>&nbsp;Reffer to Accounting&nbsp;</span>";}
else if($totac>0.001){echo "<span class='style3'>&nbsp;Balance of ".number_format($totac,2)." (Reffer to Accounting)&nbsp;</span>";}*/
...
echo "<th bgcolor='#3b5998'>&nbsp;<font color=white>Expiration</font>&nbsp;</th>";
...
//1 Find If Item is Excempted
$ipaesql=mysql_query("SELECT description FROM inventorypaexception WHERE description='".$row['description']."' AND status='Active'");
$ipaecount=mysql_num_rows($ipaesql);

if($ipaecount==0){
$pasql=mysql_query("SELECT mstatus, mopd, mopdhmo, mipd, mbaseprice FROM priceadjustments WHERE mstatus='on'");
$pacount=mysql_num_rows($pasql);
  if($pacount==0){$test12=1;
  $sp=$priceOption[1];
  $pcsp=$priceOption[1];
  }
  else{
  while($pafetch=mysql_fetch_array($pasql)){$mopd=$pafetch['mopd'];$mopdhmo=$pafetch['mopdhmo'];$mipd=$pafetch['mipd'];$mbaseprice=$pafetch['mbaseprice'];}
    if($mbaseprice=="sellingprice"){$baseprice=$priceOption[1];}else if($mbaseprice=="unitprice"){$baseprice=$row['unitcost'];}
    if($this->selectNow("registrationDetails","type","registrationNo",$registrationNo)=="OPD"){
      if($this->selectNow("registrationDetails","Company","registrationNo",$registrationNo)!=""){$test12=2;
      $sp=$baseprice+($baseprice*$mopdhmo);
      $pcsp=$baseprice+($baseprice*$mopd);
      }
      else{$test12=3;
      $sp=$baseprice+($baseprice*$mopd);
      $pcsp=$baseprice+($baseprice*$mopd);
      }
    }
    else{$test12=4;
    $sp=$baseprice+($baseprice*$mipd);
    $pcsp=$baseprice+($baseprice*$mopd);
    }
...
echo "<td>&nbsp;".number_format($sp,2)."&nbsp;</td>"; //without addons 
$this->med_sp = $sp;//without addons
...
echo "<td><font size=2 color=red>&nbsp;".$sp."&nbsp;</font></td>";
...
$limitdate = strtotime(date("Y-m-d", strtotime($expiration)) . " -1 month");

$expirycompare=date("Ymd");
$limitdatefmt=date("Ymd",$limitdate);

if($limitdatefmt<=$expirycompare){
echo "<td><font size='2' color='red'>&nbsp;$expirationfmt&nbsp;</font></td>";
}
else{
echo "<td><font size='2'>&nbsp;$expirationfmt&nbsp;</font></td>";
}
...
echo "<td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/availableMedicine/cashQTY.php?status=UNPAID&registrationNo=$registrationNo&chargesCode=$row[inventoryCode]&description=$row[description]&sellingPrice=".$pcsp."&timeCharge=$serverTime&chargeBy=$username&service=Medication&title=MEDICINE&paidVia=Cash&cashPaid=0&batchNo=$batchNo&username=$username&inventoryFrom=$searchFrom&discount=0&room=$room&paycash=yes'><font color=red size='2'>Pay Cash</font></a>&nbsp;";
}else {
...
$result = mysql_query("SELECT inventoryCode,description,unitcost,quantity,chargeControl,inventoryLocation,suppliesUNITCOST FROM inventory WHERE $searchBy like '%%%%%%%%$searchDesc%%%%%%%' and inventoryType = 'supplies' and inventoryLocation = '$searchFrom' and quantity > 0 and status not like 'DELETED_%%%%%%' order by $searchBy asc ");
...
if($ipaecount==0){
$pasql=mysql_query("SELECT sstatus, sopd, sopdhmo, sipd, sbaseprice FROM priceadjustments WHERE sstatus='on'");
$pacount=mysql_num_rows($pasql);
  if($pacount==0){
  $sp=$row['unitcost'];
  $pcsp=$row['unitcost'];
  ...
  while($pafetch=mysql_fetch_array($pasql)){$sopd=$pafetch['sopd'];$sopdhmo=$pafetch['sopdhmo'];$sipd=$pafetch['sipd'];$sbaseprice=$pafetch['sbaseprice'];}
    if($sbaseprice=="sellingprice"){$baseprice=$row['unitcost'];}else if($sbaseprice=="unitprice"){$baseprice=$row['suppliesUNITCOST'];}
    if($this->selectNow("registrationDetails","type","registrationNo",$registrationNo)=="OPD"){
      if($this->selectNow("registrationDetails","Company","registrationNo",$registrationNo)!=""){
      $sp=$baseprice+($baseprice*$sopdhmo);
      $pcsp=$baseprice+($baseprice*$sopd);
      }
      else{
      $sp=$baseprice+($baseprice*$sopd);
      $pcsp=$baseprice+($baseprice*$sopd);
      }
    }
    else{
    $sp=$baseprice+($baseprice*$sipd);
    $pcsp=$baseprice+($baseprice*$sopd);
    }
...
echo "<td>&nbsp;".($sp)."&nbsp;</td>"; //without addons
$this->sup_sp = $sp; //without addons
...
if( $row['inventoryLocation'] == "PHARMACY" || $row['inventoryLocation'] == "CSR" ) {
if( $row['chargeControl'] == "Allow" ) {
echo "<td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/availableSupplies/phicQTY.php?status=UNPAID&registrationNo=$registrationNo&chargesCode=$row[inventoryCode]&description=$row[description]&sellingPrice=$pcsp&timeCharge=$serverTime&chargeBy=$username&service=Medication&title=SUPPLIES&paidVia=Cash&cashPaid=0&batchNo=$batchNo&username=$username&inventoryFrom=$searchFrom&discount=0&room=&paycash=yes".$this->getRegistrationDetails_room()."'><font color=red size=2>Pay Cash</font></a>&nbsp;";
}else {
echo "<td>&nbsp;<a href='#'><font color=red size=2>[Locked]</font></a>&nbsp;";
}
}else {
echo "<td>&nbsp;<a href='http://".$this->getMyUrl()."/COCONUT/availableSupplies/phicQTY.php?status=UNPAID&registrationNo=$registrationNo&chargesCode=$row[inventoryCode]&description=$row[description]&sellingPrice=$pcsp&timeCharge=$serverTime&chargeBy=$username&service=Medication&title=SUPPLIES&paidVia=Cash&cashPaid=0&batchNo=$batchNo&username=$username&inventoryFrom=$searchFrom&discount=0&room=&paycash=no".$this->getRegistrationDetails_room()."'><font color=red size=2>Pay Cash</font></a>&nbsp;";
}
...
$result = mysql_query("SELECT rd.dateUnregistered,rd.patientNo,pr.completeName,rd.registrationNo,rd.dateRegistered,rd.branch,rd.type,rd.discount,rd.companyDiscount FROM patientRecord pr,registrationDetails rd WHERE pr.patientNo = rd.patientNo and rd.dateRegistered NOT LIKE 'DELETED_%%%%' and pr.completeName = '$completeName' order by rd.control_dateRegistered desc limit $start,$end ");
...
echo "<td><form method='post' action='patientInterface1.php' ><input id='namez' type='submit' value='".strtoupper($row['completeName'])."'>
...
$asql=mysql_query("SELECT SUM(cashUnpaid) AS unpaid, SUM(phic) AS phic, SUM(company) AS comp, SUM(cashPaid) AS paid FROM patientCharges WHERE registrationNo='".$row['registrationNo']."' AND status NOT LIKE 'DELETED_%%%%'");
while($afetch=mysql_fetch_array($asql)){$unpaid=$afetch['unpaid'];$phic=$afetch['phic'];$comp=$afetch['comp'];$paid=$afetch['paid'];}

$bsql=mysql_query("SELECT SUM(cashUnpaid) AS unpaidud, SUM(phic) AS phicud, SUM(company) AS compud, SUM(cashPaid) AS paidud FROM patientCharges WHERE registrationNo='".$row['registrationNo']."' AND status NOT LIKE 'DELETED_%%%%' AND title='MEDICINE' AND departmentStatus=''");
while($bfetch=mysql_fetch_array($bsql)){$unpaidud=$bfetch['unpaidud'];$phicud=$bfetch['phicud'];$compud=$bfetch['compud'];$paidud=$bfetch['paidud'];}

$csql=mysql_query("SELECT SUM(amountPaid) patAP FROM patientPayment WHERE registrationNo='".$row['registrationNo']."' AND paymentFor NOT LIKE 'REFUND'");
while($cfetch=mysql_fetch_array($csql)){$patAP=$cfetch['patAP'];}

$dsql=mysql_query("SELECT SUM(amountPaid) comAP, SUM(tax) AS comTAX, SUM(discount) AS comDISC FROM companyPayment WHERE registrationNo='".$row['registrationNo']."' AND status NOT LIKE 'DELETED_%%%%'");
while($dfetch=mysql_fetch_array($dsql)){$comAP=$dfetch['comAP'];$comTAX=$dfetch['comTAX'];$comDISC=$dfetch['comDISC'];}

$esql=mysql_query("SELECT SUM(amountPaid) phicAP, SUM(tax) AS phicTAX FROM phicPayment WHERE registrationNo='".$row['registrationNo']."' AND status NOT LIKE 'DELETED_%%%%'");
while($efetch=mysql_fetch_array($esql)){$phicAP=$efetch['phicAP'];$phicTAX=$efetch['phicTAX'];}

echo "<td>
";
$disc=$row['discount'];
if($disc==''){$realdisc=0;}else{$realdisc=$disc;}

$refund = $this->doubleSelectNow("patientPayment","amountPaid","registrationNo",$row['registrationNo'],"paymentFor","REFUND");
if($refund==''){$realrefund=0;}else{$realrefund=$refund;}

$zsql=mysql_query("SELECT module FROM registeredUser WHERE username='$username' AND (module='ADMIN' OR module='BILLING' OR module='CASHIER' OR module='PHARMACY' OR module='PHILHEALTH' OR module='SUPERVISOR')");
$zcount=mysql_num_rows($zsql);

$runpaid=round($unpaid,2);
$runpaidud=round($unpaidud,2);
$rrealdisc=round($realdisc,2);
$rpatAP=round($patAP,2);
$rrealrefund=round($realrefund,2);

if($zcount==0){
if((($runpaid-$runpaidud-$rrealdisc)-($rpatAP-$rrealrefund))==0){echo "<span class='style2'>&nbsp;Fully Paid&nbsp;</span>";}
else if((($runpaid-$runpaidud-$rrealdisc)-($rpatAP-$rrealrefund))<0){echo "<span class='style2'>&nbsp;Inquire at Accounting&nbsp;</span>";}
else if((($runpaid-$runpaidud-$rrealdisc)-($rpatAP-$rrealrefund))>0.001){echo "<span class='style3'>&nbsp;Balance&nbsp;</span>";}
}
else if($zcount!=0){
if($unpaid!=0){echo "<span class='style1'>&nbsp;Charged: </span><span class='style2'>".number_format((($unpaid-$unpaidud)-$realdisc),2)."</span>";}
if($patAP!=0){echo "<span class='style1'>&nbsp;Paid: </span><span class='style3'>".number_format(($patAP-$realrefund),2)."</span>";}
if(is_numeric($row['discount'])){echo "<span class='style1'>Disc.: </span><span class='style3'>".number_format($row['discount'],2)."</span>";}
if((!is_numeric($row['discount']))&&($patAP==0)&&($unpaid==0)){}else{echo "<br>";}
...
.style4 {font-family: Arial;font-size: 10px;color: #FF0000;}
</style>";
...
echo "<td><span class='style2'>&nbsp;".$row['description']."</span><br><span class='style4'>&nbsp;$teststat&nbsp;</span></td>";
...
.Arial12Black {font-family: Arial;font-size: 11px;font-weight:}
.Arial12BlackBold {font-family: Arial;font-size: 11px;font-weight: bold;color: #000000;}
.Arial12BlackBoldNoDeco {font-family: Arial;font-size: 11px;font-weight: bold;color: #000000;}
.Arial14Black {font-family: Arial;font-size: 14px;font-weight:}
...
echo "<td><div align='left' class='Arial12Black'>&nbsp;".$row['description']."&nbsp;</div></tD>";
echo "<td><div align='right' class='Arial12Black'>&nbsp;".number_format($row['total'],2)."&nbsp;</div></tD>";
echo "<td><div align='right' class='Arial12Black'>&nbsp;".number_format($row['phic'],2)."&nbsp;</div></tD>";
echo "<td><div align='right' class='Arial12Black'>&nbsp;".number_format($row['company'],2)."&nbsp;</div></tD>";
echo "<td><div align='right' class='Arial12Black'>&nbsp;".number_format($row['cashUnpaid'],2)."&nbsp;</div></tD>";
...
$result = mysql_query("SELECT registrationNo,room from registrationDetails WHERE room = '$roomNo' and dateUnregistered = '' and dateRegistered not like 'DELETED%%%%%'  ");
...
echo "<tD><div align='left'><span class='Arial11BlackBold'>&nbsp;".$row['paymentFor']."</span><br><span class='Arial10Blue'>&nbsp;OR#:".$row['orNo']." - ($row[datePaid])&nbsp;</span></div></td>";
echo "<TD></tD>";
echo "<TD></tD>";
echo "<TD></tD>";
echo "<TD><div align='right'><span class='Arial11Black'>&nbsp;".number_format($row['total'],2)."&nbsp;</span></div></tD>";
echo "</tr>";
}else {
echo "<tr>";
$this->descPartialPayment_total += $row['total'];
if( $this->selectNow("registeredUser","module","username",$username) == "CASHIER" ) {
echo "<tD><div align='left'><span class='Arial11BlackBold'>&nbsp;". $row['paymentFor']."</span><br><span class='Arial10Blue'>&nbsp;OR#:".$row['orNo']." - ($row[datePaid])</span><br><span class='Arial10Red'>&nbsp;Credit Card#: ".$this->ENCRYPT_DECRYPT($row['creditCardNo'])."&nbsp;</span></div></td>";
}else {
echo "<tD><div align='left'><span class='Arial11BlackBold'>&nbsp;".$row['paymentFor']."</span><br><span class='Arial10Blue'>&nbsp;OR#:".$row['orNo']."</span><br><span class='Arial10Red'>&nbsp;Credit Card#: xx-xx-xx&nbsp;</span></div></td>";
}
echo "<TD></tD>";
echo "<TD></tD>";
echo "<TD></tD>";
echo "<TD><div align='right' class='Arial11Black'>".number_format($row['total'],2)."</di></tD>";
echo "</tr>";
} 
...
//$result = mysql_query("SELECT * FROM patientCharges where registrationNo='$registrationNo' and status not like 'DELETED_%%%%%%%%' and batchNo='$batchNo' order by description asc  ");

$result = mysql_query("SELECT * FROM patientCharges where registrationNo='$registrationNo' and status not like 'DELETED_%%%%%%%%' and batchNo='$batchNo' and pendingDelete = '' order by description asc  ");
...
if(($row['title']=='LABORATORY')||($row['title']=='RADIOLOGY')){
echo "<td><a href='http://".$this->getMyUrl()."/COCONUT/patientProfile/ECART/reasonBeforeDeleteCart.php?registrationNo=$registrationNo&batchNo=$batchNo&itemNo=$row[itemNo]&username=$username'><img src='http://".$this->getMyUrl()."/COCONUT/myImages/delete.jpeg'></a></td>";
$this->coconutTableData($row['description']);
}
else{
echo "<td><a href='http://".$this->getMyUrl()."/COCONUT/patientProfile/ECART/deleteCart.php?registrationNo=$registrationNo&batchNo=$batchNo&itemNo=$row[itemNo]&username=$username&remarks='><img src='http://".$this->getMyUrl()."/COCONUT/myImages/delete.jpeg'></a></td>";
$this->coconutTableData($row['description']);
}
...
//Pending delete
public function getRequestToDeletePending($username,$registrationNo) {
...
if($registrationNo=="All"){
$result = mysql_query("SELECT * FROM patientCharges WHERE pendingDelete LIKE 'PENDING_%%%%' ORDER BY description");
}
else{
$result = mysql_query("SELECT * FROM patientCharges WHERE pendingDelete LIKE 'PENDING_%%%%' AND registrationNo='$registrationNo' ORDER BY description");
}

$asql=mysql_query("SELECT position FROM registeredUser WHERE username='$username' AND position='Supervisor'");
$acount=mysql_num_rows($asql);
...
$patientNo=$this->selectNow("registrationDetails","patientNo","registrationNo",$regNo);
$patientName=$this->selectNow("patientRecord","completeName","patientNo",$patientNo);

$this->coconutTableRowStart();
$descriptionZ = preg_split ("/\-/", $row['description']); 
$this->coconutTableData(strtoupper($patientName));
$this->coconutTableData($row['description']);
$this->coconutTableData($row['quantity']);
$this->coconutTableData($completeName[$num]);
$this->coconutTableData($row['deleteRemarks']);
if($acount!=0){
$this->coconutTableData("<a href='../../Forms/DeletePendingChargesConf.php?itemNo=$itemNo&username=$username&registrationNo=$registrationNo'><img src='http://".$this->getMyUrl()."/COCONUT/myImages/check.jpeg'></a>");
}
$this->coconutTableData("<a href='../../Forms/CancelPendingChargesConf.php?itemNo=$itemNo&username=$username&registrationNo=$registrationNo'><img src='http://".$this->getMyUrl()."/COCONUT/myImages/delete.jpeg'></a>");
$this->coconutTableRowStop();
  }
...
$result = mysql_query("SELECT upper(pr.lastName) as lastName,upper(pr.firstName) as firstName,rd.*,pp.*,rd.discount from patientRecord pr,registrationDetails rd,patientPayment pp WHERE pr.patientNo = rd.patientNo and rd.registrationNo = pp.registrationNo and (pp.datePaid between '$fromDate' and '$toDate') and pp.orNo != '' order by rd.registrationNo asc  "); 
...
$amountPd = ( $row['amountPaid'] );
$this->collectionIPD_paid += $amountPd;


$this->coconutTableData($row['registrationNo']."-".$row['lastName'].", ".$row['firstName']);
...
if($title != "") {
$result = mysql_query("SELECT sum($cols) as cols FROM patientCharges where registrationNo = '$registrationNo' and title='$title' and status = 'UNPAID' and batchNo = '$batchNo' and pendingDelete = '' ");
}else {
$result = mysql_query("SELECT sum($cols) as cols FROM patientCharges where registrationNo = '$registrationNo' and status = 'UNPAID' and batchNo = '$batchNo' and pendingDelete = '' ");
}
...
$result = mysql_query("SELECT description from patientCharges where registrationNo = '$registrationNo' and service like '%$service%' and (status = 'UNPAID' or status = 'PAID' ) ");
