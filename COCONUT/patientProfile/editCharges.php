...
}else if($ro->selectNow("patientCharges","status","itemNo",$itemNo) == "Return" ) {
echo "<td><input type=text class='shortField' name='quantity' value='".$ro->patientCharges_quantity()."' readonly></td>";
}else {
...
if( $ro->selectNow("patientCharges","title","itemNo",$itemNo) == "RADIOLOGY" ) {
echo "<tr>";
echo "<td>&nbsp;</td>";
echo "<td><a href='http://".$ro->getMyUrl()."/COCONUT/uploader/multiplefileupload.php?username=".$username."-".$ro->selectNow("patientCharges","registrationNo","itemNo",$itemNo)."-".$itemNo."' style='text-decoration:none; color:red;'><font size=2>Upload Dicom Files</font></a></td>";
echo "</tr>";
}else { }
