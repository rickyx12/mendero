...
$result = mysqli_query($connection, " select registrationNo from patientCharges where chargesCode = '$doctorCode' and title = 'PROFESSIONAL FEE' and service = 'ATTENDING' and status not like 'DELETED_%%%%' ") or die("Query fail: " . mysqli_error()); 

while($row = mysqli_fetch_array($result))
{
//if( $result->num_rows > 1 ) {
$this->getPxNameBasedOnDateCharged($date,$date1,$type,$row['registrationNo'],$type);
//echo "1";
//}else { echo "2"; }
}
...
public function getPxChargesByHorizontal($date,$date1,$type,$registrationNo) {

$connection = mysqli_connect($this->myHost,$this->username,$this->password,$this->database);      


$result = mysqli_query($connection, " select pc.description from patientRecord pr,registrationDetails rd,patientCharges pc where pc.registrationNo = '$registrationNo' and pc.registrationNo = rd.registrationNo and rd.patientNo = pr.patientNo and (pc.dateCharge between '$date' and '$date1') and rd.type='$type' and pc.title='LABORATORY' and pc.status not like 'DELETED_%%%%%%' ") or die("Query fail: " . mysqli_error()); 
...
public function getPxNameBasedOnDateCharged($date,$date1,$type,$registrationNo,$type) {

$connection = mysqli_connect($this->myHost,$this->username,$this->password,$this->database);      


$result = mysqli_query($connection, " select pr.lastName,pr.firstName,pr.middleName,pc.description,pc.dateCharge from patientRecord pr,registrationDetails rd,patientCharges pc where pc.registrationNo = '$registrationNo' and pc.registrationNo = rd.registrationNo and rd.patientNo = pr.patientNo and (pc.dateCharge between '$date' and '$date1') and rd.type='$type' and pc.title='LABORATORY' and pc.status not like 'DELETED_%%%%%%' group by rd.registrationNo ") or die("Query fail: " . mysqli_error()); 


while($row = mysqli_fetch_array($result))
  {
$this->coconutTableRowStart();
$this->coconutTableData("<font size=2>".$row['lastName'].", ".$row['firstName']." ".$row['middleName']."</font>");
$this->coconutTableData( $this->getPxChargesByHorizontal($date,$date1,$type,$registrationNo) );
$this->coconutTableRowStop();
}
