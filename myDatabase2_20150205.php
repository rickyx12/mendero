...
$result = mysql_query("SELECT itemNo,description,sellingPrice FROM patientCharges WHERE registrationNo = '$registrationNo' and (title = 'LABORATORY' or title = 'MEDICINE' or title = 'RADIOLOGY' or title = 'ECG') and status = 'UNPAID' and discount < 1 ");
...
$this->coconutTableData("<span class='txtSize'>".$row['company']."<br>".$row['paymentFor']." <br><font size=2 color=blue>REF#".$row['refNo']." w/ tax=$row[tax]</font><br><font size=2 color='blue'>Disc=$row[discount]</font></span>");
}else {
$this->coconutTableData("<span class='txtSize'>".$row['company']."<br>".$row['doctor']."<br>".$row['paymentFor']." <br><font size=2 color=blue>REF#".$row['refNo']." w/ tax=$row[tax]</font><br><font size=2 color='blue'>Disc=$row[discount]</font></span>");
}

$this->coconutTableData("&nbsp;");
$this->coconutTableData("&nbsp;");
$this->coconutTableData("&nbsp;<span class='txtSize'>".number_format(($row['amountPaid']),2)."</span>" );
$this->coconutTableData("&nbsp;");
echo "</tr>";
}
...
if( $row['paymentFor'] == "HOSPITAL BILL" ) {
$this->coconutTableData("&nbsp;<span class='txtSize2'>".number_format(( $this->summaryCompany_hospitalBill($registrationNo,$row['columnToGet']) - $this->summaryCompanyPayment_hospitalBill($registrationNo) ),2)."</span>");
}else {
...
echo "<tr>";
$this->coconutTableData("PhilHealth<br><font size=2 color=blue>REF#".$row['refNo']." w/ tax=$row[tax]</font>");
$this->coconutTableData("&nbsp;");
...
$result = mysqli_query($connection, " select sum(amountPaid) as paid from companyPayment where registrationNo = '$registrationNo' and paymentFor = 'HOSPITAL BILL' and status not like 'DELETED%%%%%%%%%' ") or die("Query fail: " . mysqli_error()); 
